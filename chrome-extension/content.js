(() => {
  ...
  for (const [key,value] of Object.entries({position:"fixed",zIndex:"2147483647",display:"none",margin:"0",padding:"0",border:"0",background:"transparent",colorScheme:"light"})) {
    host.style.setProperty(key.replace(/[A-Z]/g, c => "-" + c.toLowerCase()), value, "important");
  }
  const root = host.attachShadow({mode: "closed"});
  const style = document.createElement("style");
  style.textContent = `
    :host{all:initial} *{box-sizing:border-box} .card{font:14px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#232924;background:#fcfcf8;border:1px solid #dce4da;border-radius:16px;box-shadow:0 12px 40px #142e202e;width:400px;max-width:calc(100vw - 24px);max-height:calc(100vh - 24px);overflow:auto;text-align:left;direction:ltr;color-scheme:light}
