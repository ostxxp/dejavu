    ...
    var body: some View {
        Form {
            ...
            Section("Конфиденциальность") {
                ...
                Text("Буфер обмена проверяется только при включённом разборе скопированного. Микрофон используется только по нажатию «Ответить голосом» в мини-диалоге. Распознавание локальное; перед отправкой в ИИ можно проверить текст. Страницы браузера доступны только отдельно установленному расширению.")
                Button("Очистить историю…", role: .destructive) { clearConfirmation = true }
            }
            Section("О приложении") {
                LabeledContent("Приложение", value: "DéjàVu")
                LabeledContent("Версия", value: "0.9.0")
