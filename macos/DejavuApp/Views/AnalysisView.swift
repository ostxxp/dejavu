...
struct AnalysisView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            points(compact ? Array(analysis.grammar.prefix(2)) : analysis.grammar, title: "Как это устроено")
            if !compact { points(analysis.chunks, title: "Полезные конструкции") }
            if !analysis.examples.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Примеры").font(.headline)
                    ForEach(Array((compact ? Array(analysis.examples.prefix(1)) : analysis.examples).enumerated()), id: \.offset) { _, example in
                        VStack(alignment: .leading, spacing: 4) {
                            HStack(alignment: .firstTextBaseline) {
                                Text(example.fr)
                                Button { listen(example.fr) } label: { Image(systemName: "speaker.wave.2") }
                                    .buttonStyle(.plain).help("Прослушать пример")
                                    .accessibilityLabel("Прослушать пример: " + example.fr)
                            }
            ...
            if !compact, let notes = analysis.naturalnessNotes { Text(notes).foregroundStyle(.secondary) }
            if showsActions {
                HStack {
                    Button("Прослушать", systemImage: "speaker.wave.2") { listen(analysis.original) }
    ...
    private func listen(_ text: String) {
        do { try app.speech.speak(text); errorMessage = nil }
        catch { errorMessage = AppError.message(for: error) }
    }
